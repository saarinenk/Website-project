    ...
    Fall.prototype = {
        ...
        create: function () {
            this.stage.backgroundColor = "#ab6e0f";
            this.map = this.add.tilemap('fall');
            ...
            var passenger1 = this.humans.create(32,160, 'human', null);
            var passenger2 = this.humans.create(96,224, 'human', null);
            var passenger3 = this.humans.create(96,416, 'human', null);
            var passenger4 = this.humans.create(288,128, 'human', null);
            var passenger5 = this.humans.create(352,32, 'human', null);
            var passenger6 = this.humans.create(480,96, 'human', null);
            ...
            passenger6.loadTexture('transparent');
            
            this.move(Phaser.LEFT);
            
            this.add.button(Fall.GAME_WIDTH-50-0, 0, 'pauseb', this.managePause, this);
            ...
            var obstacle1 = this.obs.create(480, 160, 'obstacle', null);
            obstacle1.loadTexture('cone_horizontal');
            obstacle1.body.immovable = true;
            
            var obstacle2 = this.obs.create(384, 96, 'obstacle', null);
            obstacle2.loadTexture('cone_horizontal');
            obstacle2.body.immovable = true;
            
            this.scoreText = this.add.text(460, 5, "Passengers: " + this.score.toString() + "/6", {
                font: "18px Orbitron",
                fill: "#ffffff",
                align: "center",
                stroke: "black",
            ...
            this.counterText = this.add.text(225, 5, "Pollution: " + (100 - this.counter).toString() + "%", {
                font: "18px Orbitron",
                fill: "#ffffff",
                align: "center",
                stroke: "black",
            ...
            this.time.events.loop(Phaser.Timer.SECOND * 0.3, this.updateCounter, this);
        ...
        pickup: function (car, human) { //hh
            ...
            human.kill();
            this.score++;
            this.scoreText.setText("Passengers: " + this.score.toString() + "/6")
            if(this.score === 6) {
        ...
        update: function () {
            ...
            {
                this.add.sprite(0, 160, 'bs');
            }
            if (this.directions[Phaser.UP].index === 76)
            {
                this.add.sprite(96, 384, 'bs');
            }
            if (this.directions[Phaser.UP].index === 106)
            {
                this.add.sprite(96, 192, 'bs');
            }
            if (this.directions[Phaser.RIGHT].index === 76)
            {
                this.add.sprite(320, 128, 'bs');
            }
            if (this.directions[Phaser.RIGHT].index === 106)
            {
                this.add.sprite(384, 32, 'bs');
            }
            if (this.directions[Phaser.RIGHT].index === 77)
            {
                this.add.sprite(512, 96, 'bs');
            }
